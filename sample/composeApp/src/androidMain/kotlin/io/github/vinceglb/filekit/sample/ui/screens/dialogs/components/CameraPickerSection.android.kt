...
actual fun CameraPickerSection(
    ...
    var authority by remember { mutableStateOf(FileKitOpenCameraSettings.createDefault().authority) }

    val launcher = rememberCameraPickerLauncher(
        openCameraSettings = FileKitOpenCameraSettings(authority = authority),
    ) { file ->
        onPhotoCapture(file)
    }
    ...
    FeatureCard(title = "Camera picker", modifier = modifier) {
        Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            OutlinedTextField(
                ...
                label = { Text("FileProvider authority (custom destination)") },

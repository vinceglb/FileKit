...
On iOS, the suspending `FileKit.openCameraPicker()` function throws the same `FileKitDialogException` for operational failures. Android's lifecycle-safe Compose launcher owns the Android launch-failure callback behavior described above. The compatibility Compose overload without `onError` remains available and ignores normalized operational failures without logging. New integrations should use explicit error handling.

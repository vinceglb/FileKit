...
The short Compose form above is kept for source and binary compatibility. It ignores operational picker failures without logging.
For new integrations, provide `onError` explicitly:

```kotlin
val launcher = rememberFilePickerLauncher(
    onError = { failure ->
        // The valid picker operation could not be completed.
        println("Picker failed: ${failure.message}")
    },
    onResult = { file ->
        // A file was selected, or the user cancelled when file is null.
    },
)
```
